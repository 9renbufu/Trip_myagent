...
from app.reactagent.simple_agent import SimpleAgent
...
class MultiAgentTripPlanner:
    def __init__(self):
        print("MultiAgentTripPlanner 初始化")
        try:
            ...
            # 创建共享的MCP工具(只创建一次)
            print("创建MCP客户端")
            # 使用 MultiServerMCPClient 统一管理多个 MCP 服务
            ...
            # 由于当前可能已经在事件循环中（例如 FastAPI 启动时），直接使用 asyncio.run 会报错
            # 这里尝试获取当前事件循环，如果有则使用 run_until_complete，没有则直接 run
            try:
                loop = asyncio.get_running_loop()
                # 如果我们在协程中，这里应该用不同的方式，但 __init__ 是同步的
                # 所以最好使用一个独立的线程或新的事件循环来运行
                import threading
                def run_coro(coro):
                    result = []
                    def run():
                    ...
                    thread.join()
                    return result[0]
                self.amap_tool = run_coro(client.get_tools())
            except RuntimeError:
                self.amap_tool = asyncio.run(client.get_tools())
