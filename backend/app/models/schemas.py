...
class TripRequest(BaseModel):
    """定义旅行请求的请求数据结构"""
    city: str = Field(..., description="旅行城市",example="北京")
    start_date: date = Field(..., description="旅行开始日期 (格式: YYYY-MM-DD)",example="2026-04-01")
    end_date: date = Field(..., description="旅行结束日期 (格式: YYYY-MM-DD)",example="2026-04-05")
    travel_days: int = Field(..., description="旅行天数", ge=1, le=30, example=3)
    transportation: str = Field(..., description="交通方式", example="公共交通")
    accommodation: str = Field(..., description="住宿方式", example="酒店")
    preferences: List[str] = Field(default=[], description="用户偏好", example=["美食", "历史"])
    free_text_input: Optional[str] = Field(default="", description="额外要求", example="希望多安排一些历史景点")
